...
public class SingleTreeNode
{
    ...
    public SingleTreeNode(YoloState state, SingleTreeNode parent, Random rnd) {
        this.state = state;
        this.parent = parent;
        this.m_rnd = rnd;
        children = new SingleTreeNode[YoloMCTSAgent.NUM_ACTIONS];
        totValue = 0.0;
        if(parent != null)
    ...
    public SingleTreeNode treePolicy() {
        ...
        while (!cur.state.isGameOver() && cur.m_depth < YoloMCTSAgent.ROLLOUT_DEPTH)
        {
    ...
    public SingleTreeNode expand() {
        ...
        YoloState nextState = state.copy();
        nextState.advance(YoloMCTSAgent.actions[bestAction]);
    ...
    public SingleTreeNode uct() {
        ...
        for (SingleTreeNode child : this.children)
        {
            ...
            double uctValue = childValue +
                    YoloMCTSAgent.K * Math.sqrt(Math.log(this.nVisits + 1) / (child.nVisits + this.epsilon)) +
    ...
    {
        YoloState rollerState = state.copy();
        int thisDepth = this.m_depth;

        while (!finishRollout(rollerState,thisDepth)) {

            int action = m_rnd.nextInt(YoloMCTSAgent.NUM_ACTIONS);
            rollerState.advance(YoloMCTSAgent.actions[action]);
            thisDepth++;
        }
    ...
    public double value(YoloState a_gameState) {
    ...
    public boolean finishRollout(YoloState rollerState, int depth)
    {
        if(depth >= YoloMCTSAgent.ROLLOUT_DEPTH)      //rollout end condition.
            return true;
