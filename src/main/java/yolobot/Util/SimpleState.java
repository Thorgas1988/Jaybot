...
public class SimpleState {
	...
	public boolean isItypeIndexAt(int iTypeIndex, int x, int y) {
		init(x, y);
		return ((state[x][y] >> iTypeIndex) & 1) == 1;
	}

	public boolean isItypeAt(int iType, int x, int y) {
		byte index = YoloKnowledge.instance.itypeToIndex(iType);
		return isItypeIndexAt(index, x, y);
	}
