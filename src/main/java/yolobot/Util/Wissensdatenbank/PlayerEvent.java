...
public class PlayerEvent implements YoloEventController {
	...
	void learnCancelEvent(byte[] inventoryItems, boolean canceled) {
		observeCount++;
		if (canceled)
			cancelCount++;
	...
	void learnEventHappened(byte[] inventoryItems, byte newItype, boolean move, byte scoreDelta, boolean killed, byte spawnedItype, byte teleportTo, boolean winGame, byte addInventory, byte removeInventory) {
		eventCount++;
	...
	void update(byte[] inventory, boolean kill) {
		observeCount++;
